<?php

declare (strict_types=1);

namespace GianTiaga\SpiralOpenApi\Parser;

use PhpParser\Node;
use PhpParser\Error;
use PhpParser\Node\Attribute;
use PhpParser\Node\Expr;
use PhpParser\Node\Name;
use PhpParser\NodeFinder;
use PhpParser\Node\Scalar;
use PhpParser\Node\Stmt;
use PhpParser\NodeTraverser;
use PhpParser\NodeVisitor\NameResolver;
use PhpParser\ParserFactory;
use GianTiaga\SpiralOpenApi\Exception\OpenApiGenerationException;
use GianTiaga\SpiralOpenApi\Model\AdditionalResponseMetadata;
use GianTiaga\SpiralOpenApi\Model\ClassMetadata;
use GianTiaga\SpiralOpenApi\Model\FileResponseMetadata;
use GianTiaga\SpiralOpenApi\Model\MethodMetadata;
use GianTiaga\SpiralOpenApi\Model\OpenApiMetadata;
use GianTiaga\SpiralOpenApi\Model\ParameterMetadata;
use GianTiaga\SpiralOpenApi\Model\PropertyMetadata;
use GianTiaga\SpiralOpenApi\Model\RequestBodyMetadata;
use GianTiaga\SpiralOpenApi\Model\RouteMetadata;
use GianTiaga\SpiralOpenApi\Model\SourceFile;
use GianTiaga\SpiralOpenApi\Response\DataResponse;
use GianTiaga\SpiralOpenApi\Response\Enum\ContentType;
use GianTiaga\SpiralOpenApi\Response\Enum\HttpStatus;

final readonly class PhpAstParser
{
    public function __construct(private PhpDocReturnParser $phpDocReturnParser = new PhpDocReturnParser()) {}
    /**
     * @param list<SourceFile> $sourceFiles
     * @return list<ClassMetadata>
     */
    public function parse(array $sourceFiles, string $apiNamespace): array
    {
        $classes = [];
        foreach ($sourceFiles as $sourceFile) {
            foreach ($this->parseFile(sourceFile: $sourceFile, apiNamespace: $apiNamespace) as $classMetadata) {
                $classes[] = $classMetadata;
            }
        }
        return $classes;
    }
    /**
     * @return list<ClassMetadata>
     */
    private function parseFile(SourceFile $sourceFile, string $apiNamespace): array
    {
        $parser = (new ParserFactory())->createForHostVersion();
        try {
            $statements = $parser->parse((string) \file_get_contents($sourceFile->path));
        } catch (Error) {
            throw new OpenApiGenerationException(\sprintf('Не удалось разобрать PHP-файл: %s.', $sourceFile->path));
        }
        if ($statements === null) {
            throw new OpenApiGenerationException(\sprintf('Не удалось разобрать PHP-файл: %s.', $sourceFile->path));
        }
        $traverser = new NodeTraverser();
        $traverser->addVisitor(new NameResolver());
        $statements = $traverser->traverse($statements);
        return $this->parseStatements(statements: \array_values(array: $statements), sourceFile: $sourceFile, apiNamespace: \trim(string: $apiNamespace, characters: '\\'));
    }
    /**
     * @param list<Node> $statements
     * @return list<ClassMetadata>
     */
    private function parseStatements(array $statements, SourceFile $sourceFile, string $apiNamespace): array
    {
        $classes = [];
        foreach ($statements as $statement) {
            if (!$statement instanceof Stmt\Namespace_) {
                continue;
            }
            $namespace = $statement->name?->toString() ?? '';
            $aliases = $this->collectAliases(statements: \array_values(array: $statement->stmts));
            foreach ($statement->stmts as $namespaceStatement) {
                if ($namespaceStatement instanceof Stmt\Class_) {
                    $classMetadata = $this->parseClass(class: $namespaceStatement, sourceFile: $sourceFile, namespace: $namespace, aliases: $aliases);
                    if ($this->belongsToNamespace(className: $classMetadata->className, apiNamespace: $apiNamespace)) {
                        $classes[] = $classMetadata;
                    }
                }
                if ($namespaceStatement instanceof Stmt\Enum_) {
                    $classMetadata = $this->parseEnum(enum: $namespaceStatement, sourceFile: $sourceFile, namespace: $namespace);
                    if ($this->belongsToNamespace(className: $classMetadata->className, apiNamespace: $apiNamespace)) {
                        $classes[] = $classMetadata;
                    }
                }
            }
        }
        return $classes;
    }
    /**
     * @param list<Stmt> $statements
     * @return array<string, string>
     */
    private function collectAliases(array $statements): array
    {
        $aliases = [];
        foreach ($statements as $statement) {
            if (!$statement instanceof Stmt\Use_) {
                continue;
            }
            foreach ($statement->uses as $use) {
                $alias = $use->alias?->toString() ?? $use->name->getLast();
                $aliases[$alias] = $use->name->toString();
            }
        }
        return $aliases;
    }
    /**
     * @param array<string, string> $aliases
     */
    private function parseClass(Stmt\Class_ $class, SourceFile $sourceFile, string $namespace, array $aliases): ClassMetadata
    {
        $className = $this->resolvedName($class);
        return new ClassMetadata(filePath: $sourceFile->path, className: $className, shortName: $class->name?->toString() ?? $className, parentClass: $class->extends instanceof Name ? $this->resolvedName($class->extends) : null, enum: false, methods: $this->parseMethods(class: $class, aliases: $aliases, namespace: $namespace), properties: $this->parseProperties($class), enumCases: [], requestBody: $this->parseRequestBody($class));
    }
    private function parseEnum(Stmt\Enum_ $enum, SourceFile $sourceFile, string $namespace): ClassMetadata
    {
        $enumCases = [];
        foreach ($enum->stmts as $enumStatement) {
            if (!$enumStatement instanceof Stmt\EnumCase) {
                continue;
            }
            $enumCases[] = $enumStatement->expr instanceof Scalar\String_ ? $enumStatement->expr->value : $enumStatement->name->toString();
        }
        $className = $this->resolvedName($enum);
        return new ClassMetadata(filePath: $sourceFile->path, className: $className, shortName: $enum->name?->toString() ?? $className, parentClass: null, enum: true, methods: [], properties: [], enumCases: $enumCases);
    }
    private function parseRequestBody(Stmt\Class_ $class): RequestBodyMetadata|null
    {
        foreach ($class->attrGroups as $attributeGroup) {
            foreach ($attributeGroup->attrs as $attribute) {
                if (!$this->attributeIs(attribute: $attribute, className: 'GianTiaga\SpiralOpenApi\Attribute\OpenApiRequestBody')) {
                    continue;
                }
                return new RequestBodyMetadata(
                    requiredAnyOf: $this->stringListArgument(attribute: $attribute, name: 'requiredAnyOf', position: 0, default: []),
                    additionalProperties: $this->nullableBoolArgument(attribute: $attribute, name: 'additionalProperties', position: 1),
                );
            }
        }
        return null;
    }
    /**
     * @param array<string, string> $aliases
     * @return list<MethodMetadata>
     */
    private function parseMethods(Stmt\Class_ $class, array $aliases, string $namespace): array
    {
        $methods = [];
        foreach ($class->getMethods() as $method) {
            if (!$method->isPublic()) {
                continue;
            }
            $returnType = $method->returnType instanceof Node ? $this->typeName($method->returnType) : null;
            $methods[] = new MethodMetadata(
                name: $method->name->toString(),
                summary: $this->summaryFromDocComment(docComment: $method->getDocComment()?->getText()),
                returnType: $returnType,
                genericReturnType: $this->phpDocReturnParser->parse(docComment: $method->getDocComment()?->getText(), aliases: $aliases, namespace: $namespace),
                fileResponse: $this->parseFileResponse(method: $method),
                route: $this->parseRoute(method: $method),
                openApi: $this->parseOpenApi(method: $method),
                parameters: $this->parseParameters(method: $method),
                additionalResponses: $this->parseAdditionalResponses(method: $method),
                errorStatuses: $this->parseErrorStatuses(method: $method),
                attributeClasses: \array_map(
                    callback: fn(Attribute $attribute): string => $this->resolvedName($attribute->name),
                    array: $this->attributes($method),
                ),
                tag: $this->parseTag(method: $method),
            );
        }
        return $methods;
    }
    private function parseFileResponse(Stmt\ClassMethod $method): FileResponseMetadata|null
    {
        foreach ((new NodeFinder())->findInstanceOf(nodes: $method->stmts ?? [], class: Expr\New_::class) as $newExpression) {
            if (!$newExpression->class instanceof Name) {
                continue;
            }
            $responseClass = $this->resolvedName($newExpression->class);
            if ($responseClass !== 'GianTiaga\SpiralOpenApi\Response\FileContentResponse' && $responseClass !== 'GianTiaga\SpiralOpenApi\Response\FileResponse') {
                continue;
            }
            return new FileResponseMetadata(responseClass: $responseClass, contentType: $this->contentTypeArgument($newExpression), binary: $responseClass === 'GianTiaga\SpiralOpenApi\Response\FileResponse');
        }
        return null;
    }
    private function contentTypeArgument(Expr\New_ $newExpression): string
    {
        foreach ($newExpression->args as $position => $argument) {
            if (!$argument instanceof Node\Arg) {
                continue;
            }
            if ($argument->name?->toString() !== 'contentType' && ($argument->name !== null || $position !== 1)) {
                continue;
            }
            if (!$argument->value instanceof Expr\ClassConstFetch || !$argument->value->class instanceof Name) {
                break;
            }
            if ($this->resolvedName($argument->value->class) !== ContentType::class) {
                break;
            }
            $caseName = $argument->value->name instanceof Node\Identifier ? $argument->value->name->toString() : '';
            foreach (ContentType::cases() as $contentType) {
                if ($contentType->name === $caseName) {
                    return $contentType->value;
                }
            }
        }
        throw new OpenApiGenerationException('Файловый ответ должен передавать contentType через enum ContentType.');
    }
    /**
     * @return list<PropertyMetadata>
     */
    private function parseProperties(Stmt\Class_ $class): array
    {
        $properties = [];
        foreach ($class->getProperties() as $property) {
            if (!$property->isPublic()) {
                continue;
            }
            foreach ($property->props as $propertyProperty) {
                $type = $property->type instanceof Node ? $this->typeName($property->type) : 'string';
                $attributeGroups = \array_values(array: $property->attrGroups);
                $openApiProperty = $this->openApiPropertyAttribute($attributeGroups);
                $properties[] = new PropertyMetadata(
                    name: $this->inputName(attributeGroups: $attributeGroups, fallback: $propertyProperty->name->toString()),
                    type: $this->baseTypeName($type),
                    nullable: $openApiProperty === null
                        ? $this->isNullableType($property->type)
                        : $this->nullableBoolArgument(attribute: $openApiProperty, name: 'nullable', position: 7)
                            ?? $this->isNullableType($property->type),
                    hasDefault: $propertyProperty->default !== null,
                    source: $this->inputSource(attributeGroups: $attributeGroups),
                    listItemType: $openApiProperty === null
                        ? $this->listItemType($property->getDocComment()?->getText())
                        : $this->nullableStringArgument(attribute: $openApiProperty, name: 'items', position: 6)
                            ?? $this->listItemType($property->getDocComment()?->getText()),
                    unionTypes: $this->unionClassTypes($property->type),
                    format: $openApiProperty === null ? null : $this->nullableStringArgument(attribute: $openApiProperty, name: 'format', position: 0),
                    pattern: $openApiProperty === null ? null : $this->nullableStringArgument(attribute: $openApiProperty, name: 'pattern', position: 1),
                    minimum: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'minimum', position: 2),
                    maximum: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'maximum', position: 3),
                    minLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'minLength', position: 4),
                    maxLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'maxLength', position: 5),
                    uniqueItems: $openApiProperty === null ? null : $this->nullableBoolArgument(attribute: $openApiProperty, name: 'uniqueItems', position: 8),
                    itemsPattern: $openApiProperty === null ? null : $this->nullableStringArgument(attribute: $openApiProperty, name: 'itemsPattern', position: 9),
                    itemsMinLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'itemsMinLength', position: 10),
                    itemsMaxLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'itemsMaxLength', position: 11),
                    defaultValue: $this->scalarDefaultValue($propertyProperty->default),
                );
            }
        }
        foreach ($class->getMethods() as $method) {
            if ($method->name->toString() !== '__construct') {
                continue;
            }
            $constructorDocComment = $method->getDocComment()?->getText();
            foreach ($method->params as $parameter) {
                if (($parameter->flags & Stmt\Class_::MODIFIER_PUBLIC) !== Stmt\Class_::MODIFIER_PUBLIC) {
                    continue;
                }
                $type = $parameter->type instanceof Node ? $this->typeName($parameter->type) : 'string';
                $parameterName = $parameter->var instanceof Expr\Variable && \is_string($parameter->var->name) ? $parameter->var->name : 'value';
                $attributeGroups = \array_values(array: $parameter->attrGroups);
                $openApiProperty = $this->openApiPropertyAttribute($attributeGroups);
                $properties[] = new PropertyMetadata(
                    name: $parameterName,
                    type: $this->baseTypeName($type),
                    nullable: $openApiProperty === null
                        ? $this->isNullableType($parameter->type)
                        : $this->nullableBoolArgument(attribute: $openApiProperty, name: 'nullable', position: 7)
                            ?? $this->isNullableType($parameter->type),
                    hasDefault: $parameter->default !== null,
                    source: PropertyMetadata::SOURCE_NONE,
                    listItemType: $openApiProperty === null
                        ? $this->paramListItemType(docComment: $constructorDocComment, parameterName: $parameterName)
                        : $this->nullableStringArgument(attribute: $openApiProperty, name: 'items', position: 6)
                            ?? $this->paramListItemType(docComment: $constructorDocComment, parameterName: $parameterName),
                    unionTypes: $this->unionClassTypes($parameter->type),
                    format: $openApiProperty === null ? null : $this->nullableStringArgument(attribute: $openApiProperty, name: 'format', position: 0),
                    pattern: $openApiProperty === null ? null : $this->nullableStringArgument(attribute: $openApiProperty, name: 'pattern', position: 1),
                    minimum: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'minimum', position: 2),
                    maximum: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'maximum', position: 3),
                    minLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'minLength', position: 4),
                    maxLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'maxLength', position: 5),
                    uniqueItems: $openApiProperty === null ? null : $this->nullableBoolArgument(attribute: $openApiProperty, name: 'uniqueItems', position: 8),
                    itemsPattern: $openApiProperty === null ? null : $this->nullableStringArgument(attribute: $openApiProperty, name: 'itemsPattern', position: 9),
                    itemsMinLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'itemsMinLength', position: 10),
                    itemsMaxLength: $openApiProperty === null ? null : $this->nullableIntArgument(attribute: $openApiProperty, name: 'itemsMaxLength', position: 11),
                    defaultValue: $this->scalarDefaultValue($parameter->default),
                );
            }
        }
        return $properties;
    }
    /**
     * @return list<ParameterMetadata>
     */
    private function parseParameters(Stmt\ClassMethod $method): array
    {
        $parameters = [];
        foreach ($method->params as $parameter) {
            if (!$parameter->var instanceof Expr\Variable || !\is_string($parameter->var->name)) {
                continue;
            }
            $type = $parameter->type instanceof Node ? $this->typeName($parameter->type) : 'string';
            $parameters[] = new ParameterMetadata(name: $parameter->var->name, type: $this->baseTypeName($type), nullable: $this->isNullableType($parameter->type));
        }
        return $parameters;
    }
    private function parseRoute(Stmt\ClassMethod $method): RouteMetadata|null
    {
        foreach ($this->attributes($method) as $attribute) {
            if (!$this->attributeIs(attribute: $attribute, className: 'Spiral\Router\Annotation\Route')) {
                continue;
            }
            return new RouteMetadata(path: $this->stringArgument(attribute: $attribute, name: 'route', position: 0, default: ''), name: $this->nullableStringArgument(attribute: $attribute, name: 'name', position: 1), methods: $this->stringListArgument(attribute: $attribute, name: 'methods', position: 2, default: ['GET']), group: $this->nullableStringArgument(attribute: $attribute, name: 'group', position: 4), middleware: $this->stringListArgument(attribute: $attribute, name: 'middleware', position: 5, default: []), priority: $this->intArgument(attribute: $attribute, name: 'priority', position: 6, default: 0));
        }
        return null;
    }
    private function parseTag(Stmt\ClassMethod $method): string|null
    {
        foreach ($this->attributes($method) as $attribute) {
            if (!$this->attributeIs(attribute: $attribute, className: 'GianTiaga\SpiralOpenApi\Attribute\OpenApiTag')) {
                continue;
            }
            return $this->stringArgument(attribute: $attribute, name: 'name', position: 0, default: '');
        }
        return null;
    }
    private function parseOpenApi(Stmt\ClassMethod $method): OpenApiMetadata|null
    {
        foreach ($this->attributes($method) as $attribute) {
            if (!$this->attributeIs(attribute: $attribute, className: 'GianTiaga\SpiralOpenApi\Attribute\OpenApi')) {
                continue;
            }
            return new OpenApiMetadata(
                id: $this->stringArgument(attribute: $attribute, name: 'id', position: 0, default: ''),
                description: $this->stringArgument(attribute: $attribute, name: 'description', position: 1, default: ''),
                ignore: $this->boolArgument(attribute: $attribute, name: 'ignore', position: 2, default: false),
                successStatus: $this->intArgument(attribute: $attribute, name: 'successStatus', position: 3, default: HttpStatus::Ok->value),
            );
        }
        return null;
    }
    /**
     * @return list<AdditionalResponseMetadata>
     */
    private function parseAdditionalResponses(Stmt\ClassMethod $method): array
    {
        $additionalResponses = [];
        foreach ($this->attributes($method) as $attribute) {
            if (!$this->attributeIs(attribute: $attribute, className: 'GianTiaga\SpiralOpenApi\Attribute\OpenApiResponse')) {
                continue;
            }
            $additionalResponses[] = new AdditionalResponseMetadata(status: $this->httpStatusArgument(attribute: $attribute, name: 'status', position: 0), wrapperClass: $this->stringArgument(attribute: $attribute, name: 'wrapper', position: 2, default: DataResponse::class), resourceClass: $this->requiredClassArgument(attribute: $attribute, name: 'resource', position: 1), description: $this->stringArgument(attribute: $attribute, name: 'description', position: 3, default: ''));
        }
        return $additionalResponses;
    }
    /**
     * @return list<int>
     */
    private function parseErrorStatuses(Stmt\ClassMethod $method): array
    {
        foreach ($this->attributes($method) as $attribute) {
            if (!$this->attributeIs(attribute: $attribute, className: 'GianTiaga\SpiralOpenApi\Attribute\OpenApiErrorResponses')) {
                continue;
            }
            return $this->intListArgument(attribute: $attribute, name: 'statuses', position: 0);
        }
        return [];
    }
    private function httpStatusArgument(Attribute $attribute, string $name, int $position): int
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: null);
        if (!\is_int($argument)) {
            throw new OpenApiGenerationException('Дополнительный код ответа объявляется значением enum HttpStatus.');
        }
        return $argument;
    }
    private function requiredClassArgument(Attribute $attribute, string $name, int $position): string
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: null);
        if (!\is_string($argument) || $argument === '') {
            throw new OpenApiGenerationException(\sprintf('Дополнительный ответ требует аргумент %s со ссылкой на класс через ::class.', $name));
        }
        return $argument;
    }
    /**
     * @param list<Node\AttributeGroup> $attributeGroups
     */
    private function inputSource(array $attributeGroups): string
    {
        foreach ($attributeGroups as $attributeGroup) {
            foreach ($attributeGroup->attrs as $attribute) {
                $attributeName = $this->resolvedName($attribute->name);
                if (\str_ends_with(haystack: $attributeName, needle: '\Query')) {
                    return PropertyMetadata::SOURCE_QUERY;
                }
                if (\str_ends_with(haystack: $attributeName, needle: '\Route') || \str_ends_with(haystack: $attributeName, needle: '\Path')) {
                    return PropertyMetadata::SOURCE_PATH;
                }
                if (\str_ends_with(haystack: $attributeName, needle: '\Post')) {
                    return PropertyMetadata::SOURCE_BODY;
                }
                if (\str_ends_with(haystack: $attributeName, needle: '\Data') || \str_ends_with(haystack: $attributeName, needle: '\NestedFilter')) {
                    return PropertyMetadata::SOURCE_DATA;
                }
            }
        }
        return PropertyMetadata::SOURCE_NONE;
    }
    /**
     * @param list<Node\AttributeGroup> $attributeGroups
     */
    private function inputName(array $attributeGroups, string $fallback): string
    {
        foreach ($attributeGroups as $attributeGroup) {
            foreach ($attributeGroup->attrs as $attribute) {
                $key = $this->nullableStringArgument(attribute: $attribute, name: 'key', position: 0);
                if ($key !== null) {
                    return $key;
                }
            }
        }
        return $fallback;
    }
    /**
     * @return list<Attribute>
     */
    private function attributes(Stmt\ClassMethod $method): array
    {
        $attributes = [];
        foreach ($method->attrGroups as $attributeGroup) {
            foreach ($attributeGroup->attrs as $attribute) {
                $attributes[] = $attribute;
            }
        }
        return $attributes;
    }
    private function attributeIs(Attribute $attribute, string $className): bool
    {
        $attributeName = $this->resolvedName($attribute->name);
        return $attributeName === $className || \str_ends_with(haystack: $attributeName, needle: '\\' . \basename(\str_replace(search: '\\', replace: '/', subject: $className)));
    }
    private function nullableStringArgument(Attribute $attribute, string $name, int $position): string|null
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: null);
        return \is_string($argument) && $argument !== '' ? $argument : null;
    }
    private function stringArgument(Attribute $attribute, string $name, int $position, string $default): string
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: $default);
        return \is_string($argument) ? $argument : $default;
    }
    private function intArgument(Attribute $attribute, string $name, int $position, int $default): int
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: $default);
        return \is_int($argument) ? $argument : $default;
    }
    private function nullableIntArgument(Attribute $attribute, string $name, int $position): int|null
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: null);
        return \is_int($argument) ? $argument : null;
    }
    private function boolArgument(Attribute $attribute, string $name, int $position, bool $default): bool
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: $default);
        return \is_bool($argument) ? $argument : $default;
    }
    private function nullableBoolArgument(Attribute $attribute, string $name, int $position): bool|null
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: null);
        return \is_bool($argument) ? $argument : null;
    }
    /**
     * @param list<string> $default
     * @return list<string>
     */
    private function stringListArgument(Attribute $attribute, string $name, int $position, array $default): array
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: $default);
        if (\is_string($argument)) {
            return [$argument];
        }
        if (!\is_array($argument)) {
            return $default;
        }
        $values = [];
        foreach ($argument as $value) {
            if (\is_string($value)) {
                $values[] = $value;
            }
        }
        return $values;
    }
    /**
     * @return list<int>
     */
    private function intListArgument(Attribute $attribute, string $name, int $position): array
    {
        $argument = $this->argument(attribute: $attribute, name: $name, position: $position, default: []);
        if (!\is_array($argument)) {
            return [];
        }
        $values = [];
        foreach ($argument as $value) {
            if (\is_int($value)) {
                $values[] = $value;
            }
        }
        return $values;
    }
    /**
     * @param list<Node\AttributeGroup> $attributeGroups
     */
    private function openApiPropertyAttribute(array $attributeGroups): Attribute|null
    {
        foreach ($attributeGroups as $attributeGroup) {
            foreach ($attributeGroup->attrs as $attribute) {
                if ($this->attributeIs(attribute: $attribute, className: 'GianTiaga\SpiralOpenApi\Attribute\OpenApiProperty')) {
                    return $attribute;
                }
            }
        }
        return null;
    }
    private function scalarDefaultValue(Expr|null $expression): string|int|bool|null
    {
        if ($expression === null) {
            return null;
        }
        $value = $this->value($expression);
        return \is_string($value) || \is_int($value) || \is_bool($value) ? $value : null;
    }
    private function argument(Attribute $attribute, string $name, int $position, mixed $default): mixed
    {
        foreach ($attribute->args as $index => $argument) {
            if ($argument->name?->toString() === $name || $argument->name === null && $index === $position) {
                return $this->value($argument->value);
            }
        }
        return $default;
    }
    private function value(Expr $expression): mixed
    {
        if ($expression instanceof Scalar\String_) {
            return $expression->value;
        }
        if ($expression instanceof Scalar\LNumber) {
            return $expression->value;
        }
        if ($expression instanceof Expr\ConstFetch) {
            return match (\strtolower($expression->name->toString())) {
                'true' => true,
                'false' => false,
                default => null,
            };
        }
        if ($expression instanceof Expr\Array_) {
            $values = [];
            foreach ($expression->items as $item) {
                $values[] = $this->value($item->value);
            }
            return $values;
        }
        if ($expression instanceof Expr\ClassConstFetch && $expression->class instanceof Name) {
            return $this->classConstantValue($expression);
        }
        return null;
    }
    /**
     * Значение `Foo::class` и случая enum `HttpStatus`. Генератор читает исходники статически,
     * поэтому константу приходится разрешать самому: `Foo::class` даёт имя класса, а случай
     * `HttpStatus` — числовой код ответа. Остальные константы генератору неизвестны.
     */
    private function classConstantValue(Expr\ClassConstFetch $expression): string|int|null
    {
        if (!$expression->class instanceof Name) {
            return null;
        }
        $className = $this->resolvedName($expression->class);
        $constantName = $expression->name instanceof Node\Identifier ? $expression->name->toString() : '';
        if ($constantName === 'class') {
            return $className;
        }
        if ($className !== HttpStatus::class) {
            return null;
        }
        foreach (HttpStatus::cases() as $httpStatus) {
            if ($httpStatus->name === $constantName) {
                return $httpStatus->value;
            }
        }
        return null;
    }
    /**
     * Тип nullable, если это `?T` (NullableType) или union, содержащий `null`
     * (`T|null`, `null|T`). PHP допускает обе формы, генератор должен трактовать их одинаково.
     */
    private function isNullableType(Node|null $type): bool
    {
        if ($type instanceof Node\NullableType) {
            return true;
        }
        if (!$type instanceof Node\UnionType) {
            return false;
        }
        foreach ($type->types as $unionType) {
            if ($unionType instanceof Node\Identifier && \strtolower($unionType->toString()) === 'null') {
                return true;
            }
            if ($unionType instanceof Name && \strtolower($this->resolvedName($unionType)) === 'null') {
                return true;
            }
        }
        return false;
    }
    private function typeName(Node $type): string
    {
        if ($type instanceof Node\NullableType) {
            return '?' . $this->typeName($type->type);
        }
        if ($type instanceof Node\UnionType) {
            return \implode(separator: '|', array: \array_map(fn(Node $unionType): string => $this->typeName($unionType), $type->types));
        }
        if ($type instanceof Name) {
            return $this->resolvedName($type);
        }
        if ($type instanceof Node\Identifier) {
            return $type->toString();
        }
        return 'string';
    }
    private function baseTypeName(string $type): string
    {
        $parts = \explode(separator: '|', string: \str_replace(search: '?', replace: '', subject: $type));
        foreach ($parts as $part) {
            $normalizedPart = \ltrim(string: $part, characters: '\\');
            if ($normalizedPart !== '' && \strtolower($normalizedPart) !== 'null') {
                return $normalizedPart;
            }
        }
        return \ltrim(string: $parts[0], characters: '\\');
    }
    /**
     * Классы-члены объединения (union) для представления через oneOf. Пусто, если тип не union, если
     * среди членов есть скаляр (тогда достаточно baseTypeName) или если не-null класс всего один.
     *
     * @return list<string>
     */
    private function unionClassTypes(Node|null $type): array
    {
        if (!$type instanceof Node\UnionType) {
            return [];
        }
        $classTypes = [];
        foreach ($type->types as $unionType) {
            $normalizedType = \ltrim(string: $this->typeName($unionType), characters: '\\');
            if ($normalizedType === '' || \strtolower($normalizedType) === 'null') {
                continue;
            }
            if ($this->isScalarType($normalizedType)) {
                return [];
            }
            $classTypes[] = $normalizedType;
        }
        return \count($classTypes) >= 2 ? $classTypes : [];
    }
    private function isScalarType(string $type): bool
    {
        return \in_array(needle: \strtolower($type), haystack: ['string', 'int', 'integer', 'float', 'double', 'bool', 'boolean', 'array', 'mixed', 'object', 'iterable', 'callable', 'self', 'static'], strict: true);
    }
    private function resolvedName(Node $node): string
    {
        $resolvedName = $node->getAttribute('namespacedName') ?? $node->getAttribute('resolvedName');
        if ($resolvedName instanceof Name) {
            return $resolvedName->toString();
        }
        if ($node instanceof Name) {
            return $node->toString();
        }
        if ($node instanceof Stmt\Class_ || $node instanceof Stmt\Enum_) {
            if ($node->namespacedName instanceof Name) {
                return $node->namespacedName->toString();
            }
            return $node->name?->toString() ?? '';
        }
        return '';
    }
    private function belongsToNamespace(string $className, string $apiNamespace): bool
    {
        return $className === $apiNamespace || \str_starts_with(haystack: $className, needle: $apiNamespace . '\\');
    }
    private function summaryFromDocComment(string|null $docComment): string
    {
        if ($docComment === null) {
            return '';
        }
        $lines = \explode(separator: "\n", string: $docComment);
        foreach ($lines as $line) {
            $cleanLine = \trim(\str_replace(search: ['/**', '*/', '*'], replace: '', subject: $line));
            if ($cleanLine === '' || \str_starts_with(haystack: $cleanLine, needle: '@')) {
                continue;
            }
            return $cleanLine;
        }
        return '';
    }
    private function listItemType(string|null $docComment): string|null
    {
        if ($docComment === null || !\str_contains(haystack: $docComment, needle: 'list<')) {
            return null;
        }
        $start = \strpos(haystack: $docComment, needle: 'list<');
        if ($start === false) {
            return null;
        }
        $tail = \substr(string: $docComment, offset: $start + 5);
        $end = \strpos(haystack: $tail, needle: '>');
        if ($end === false) {
            return null;
        }
        return \substr(string: $tail, offset: 0, length: $end);
    }
    /**
     * Тип элемента списка для promoted-параметра конструктора. PHPDoc такого свойства живёт на
     * `@param list<T> $name` конструктора, поэтому тип элемента ищется по конкретному имени параметра,
     * а не как первый попавшийся `list<` (иначе несколько списков в одном докблоке перепутаются).
     */
    private function paramListItemType(string|null $docComment, string $parameterName): string|null
    {
        if ($docComment === null) {
            return null;
        }
        if (\preg_match(pattern: \sprintf('/@param\s+list<([^>]+)>\s+\$%s\b/', \preg_quote(str: $parameterName, delimiter: '/')), subject: $docComment, matches: $matches) !== 1) {
            return null;
        }
        return \trim(string: $matches[1]);
    }
}
