...
final class OpenApiGeneratorTest extends TestCase
{
    ...
    public function testEmptyTagFailsGeneration(): void
    {
        $this->expectException(OpenApiGenerationException::class);
        $this->expectExceptionMessage('Тег операции GianTiaga\\SpiralOpenApi\\Tests\\Fixtures\\Endpoint\\EmptyTag\\Api\\V1\\Controller\\EmptyTagController::show пуст.');
        (new OpenApiGenerator(translator: self::englishTranslator()))->generate(new OpenApiGeneratorConfig(projectRoot: __DIR__ . '/../..', sourcePaths: [__DIR__ . '/../Fixtures/Endpoint/EmptyTag/Api/V1'], apiNamespace: 'GianTiaga\SpiralOpenApi\Tests\Fixtures\Endpoint\EmptyTag\Api\V1', routePrefix: '/api/v1', outputFile: __DIR__ . '/../../runtime/openapi-empty-tag.yml', title: 'Fixture API', version: '1.0.0', responseWrapperMapping: new ResponseWrapperMapping(dataResponseClass: DataResponse::class, collectionResponseClass: CollectionResponse::class, paginationResponseClass: PaginationResponse::class, errorResponseClass: ErrorResponse::class, emptyResponseClass: EmptySuccessResponse::class)));
    }
    ...
    public function testAdditionalResponseCodeIsDeclaredWithOwnSchema(): void
    {
        $outputFile = __DIR__ . '/../../runtime/openapi-fixture-additional-response.yml';
        $result = (new OpenApiGenerator(translator: self::englishTranslator()))->generate(new OpenApiGeneratorConfig(projectRoot: __DIR__ . '/../..', sourcePaths: [__DIR__ . '/../Fixtures/Endpoint/AdditionalResponse/Api/V1'], apiNamespace: 'GianTiaga\SpiralOpenApi\Tests\Fixtures\Endpoint\AdditionalResponse\Api\V1', routePrefix: '/api/v1', outputFile: $outputFile, title: 'Fixture API', version: '1.0.0', responseWrapperMapping: new ResponseWrapperMapping(dataResponseClass: DataResponse::class, collectionResponseClass: CollectionResponse::class, paginationResponseClass: PaginationResponse::class, errorResponseClass: ErrorResponse::class, emptyResponseClass: EmptySuccessResponse::class)));
        self::assertSame(2, $result->operationCount);
        self::assertFileExists($outputFile);
        $spec = Yaml::parseFile($outputFile);
        self::assertIsArray($spec);
        $specNode = new OpenApiSpecNode(value: $spec);
        $responses = $specNode->child(key: 'paths')->child(key: '/readiness')->child(key: 'get')->child(key: 'responses');
        // Успешный ответ из @return остаётся на месте, дополнительный код добавляется рядом с ним.
        self::assertTrue($responses->has(key: 200));
        self::assertTrue($responses->has(key: 503));
        self::assertTrue($responses->has(key: 'default'));
        $unavailableResponse = $responses->child(key: 503);
        self::assertSame('Service is not ready.', $unavailableResponse->value(key: 'description'));
        // Тело дополнительного ответа описано схемой компонентов, операция только ссылается на неё.
        self::assertSame(
            '#/components/schemas/ReadinessResourceDataResponse',
            $unavailableResponse->child(key: 'content')->child(key: 'application/json')->child(key: 'schema')->value(key: '$ref'),
        );
        // Схема тела дополнительного ответа попадает в компоненты вместе со своим enum.
        $schemas = $specNode->child(key: 'components')->child(key: 'schemas');
        $unavailableSchema = $schemas->child(key: 'ReadinessResourceDataResponse');
        self::assertSame('object', $unavailableSchema->value(key: 'type'));
        self::assertSame(['data'], $unavailableSchema->value(key: 'required'));
        self::assertSame(['$ref' => '#/components/schemas/ReadinessResource'], $unavailableSchema->child(key: 'properties')->value(key: 'data'));
        self::assertTrue($schemas->has(key: 'ReadinessResource'));
        self::assertTrue($schemas->has(key: 'ReadinessStatus'));
        // Без описания в атрибуте берётся перевод пакета, а не описание успешного ответа.
        $strictResponses = $specNode->child(key: 'paths')->child(key: '/readiness/strict')->child(key: 'get')->child(key: 'responses');
        self::assertSame('Additional response.', $strictResponses->child(key: 503)->value(key: 'description'));
        self::assertSame('Successful response.', $strictResponses->child(key: 200)->value(key: 'description'));
    }
    ...
    public function testTypedContractMetadataDescribesStatusErrorsAndInputConstraints(): void
    {
        $outputFile = __DIR__ . '/../../runtime/openapi-fixture-contract-metadata.yml';
        $result = (new OpenApiGenerator(translator: self::englishTranslator()))->generate(new OpenApiGeneratorConfig(projectRoot: __DIR__ . '/../..', sourcePaths: [__DIR__ . '/../Fixtures/Endpoint/ContractMetadata/Api/V1'], apiNamespace: 'GianTiaga\SpiralOpenApi\Tests\Fixtures\Endpoint\ContractMetadata\Api\V1', routePrefix: '/api/v1', outputFile: $outputFile, title: 'Fixture API', version: '1.0.0', responseWrapperMapping: new ResponseWrapperMapping(dataResponseClass: DataResponse::class, collectionResponseClass: CollectionResponse::class, paginationResponseClass: PaginationResponse::class, errorResponseClass: ErrorResponse::class, emptyResponseClass: EmptySuccessResponse::class)));
        self::assertSame(1, $result->operationCount);
        $spec = Yaml::parseFile($outputFile);
        self::assertIsArray($spec);
        $specNode = new OpenApiSpecNode(value: $spec);
        $operation = $specNode->child(key: 'paths')->child(key: '/roles/{roleSlug}')->child(key: 'post');
        $responses = $operation->child(key: 'responses');
        self::assertTrue($responses->has(key: 201));
        self::assertFalse($responses->has(key: 200));
        self::assertTrue($responses->has(key: 401));
        self::assertTrue($responses->has(key: 409));
        self::assertTrue($responses->has(key: 422));
        $pathParameter = $operation->child(key: 'parameters')->child(key: 0);
        self::assertSame('roleSlug', $pathParameter->value(key: 'name'));
        self::assertSame('path', $pathParameter->value(key: 'in'));
        self::assertTrue($pathParameter->value(key: 'required'));
        self::assertSame('^[a-z0-9-]+$', $pathParameter->child(key: 'schema')->value(key: 'pattern'));
        self::assertSame(64, $pathParameter->child(key: 'schema')->value(key: 'maxLength'));
        $cursorParameter = $operation->child(key: 'parameters')->child(key: 1);
        self::assertSame('uuid', $cursorParameter->child(key: 'schema')->value(key: 'format'));
        self::assertSame('uuid-v7', $cursorParameter->child(key: 'schema')->value(key: 'pattern'));
        $limitSchema = $operation->child(key: 'parameters')->child(key: 2)->child(key: 'schema');
        self::assertSame(1, $limitSchema->value(key: 'minimum'));
        self::assertSame(100, $limitSchema->value(key: 'maximum'));
        self::assertSame(50, $limitSchema->value(key: 'default'));
        // Тело запроса описано схемой компонентов с именем Filter операции, операция ссылается на неё.
        self::assertSame(
            '#/components/schemas/RoleFilter',
            $operation->child(key: 'requestBody')->child(key: 'content')->child(key: 'application/json')->child(key: 'schema')->value(key: '$ref'),
        );
        $bodySchema = $specNode->child(key: 'components')->child(key: 'schemas')->child(key: 'RoleFilter');
        self::assertSame(['permissions'], $bodySchema->value(key: 'required'));
        self::assertSame(
            [['required' => ['email']], ['required' => ['name']]],
            $bodySchema->value(key: 'anyOf'),
        );
        self::assertFalse($bodySchema->value(key: 'additionalProperties'));
        $bodyProperties = $bodySchema->child(key: 'properties');
        $permissions = $bodyProperties->child(key: 'permissions');
        $permissionItems = $permissions->child(key: 'items');
        self::assertTrue($permissions->value(key: 'uniqueItems'));
        self::assertSame('#/components/schemas/RolePermission', $permissionItems->value(key: '$ref'));
        self::assertSame('^[a-z]+$', $permissionItems->value(key: 'pattern'));
        self::assertSame(1, $permissionItems->value(key: 'minLength'));
        self::assertSame(32, $permissionItems->value(key: 'maxLength'));
        $email = $bodyProperties->child(key: 'email');
        self::assertSame('string', $email->value(key: 'type'));
        self::assertSame('email', $email->value(key: 'format'));
        self::assertSame(1, $email->value(key: 'minLength'));
        self::assertFalse($email->has(key: 'nullable'));
        self::assertSame(
            1,
            $specNode
                ->child(key: 'components')
                ->child(key: 'schemas')
                ->child(key: 'RoleResource')
    ...
    public function testConfigurableAccessAttributesGenerateBearerSecurity(): void
    {
        ...
        $result = (new OpenApiGenerator(translator: self::englishTranslator()))->generate(
            new OpenApiGeneratorConfig(
                projectRoot: __DIR__ . '/../..',
                sourcePaths: [__DIR__ . '/../Fixtures/Endpoint/Security/Api/V1'],
                apiNamespace: 'GianTiaga\SpiralOpenApi\Tests\Fixtures\Endpoint\Security\Api\V1',
                routePrefix: '/api/v1',
                ...
                bearerSecurity: new BearerSecurityConfig(
                    ...
                    publicAccessAttributeClasses: [PublicAccess::class],
                    protectedAccessAttributeClasses: [BearerAccess::class],
                ),
            ),
        );
        ...
        self::assertSame(
            ...
            $specNode
                ->child(key: 'components')
                ->child(key: 'securitySchemes')
                ->value(key: 'bearerAuth'),
        );
        self::assertSame(
            [],
            $specNode->child(key: 'paths')->child(key: '/public')->child(key: 'get')->value(key: 'security'),
        );
        self::assertSame(
            [['bearerAuth' => []]],
            $specNode->child(key: 'paths')->child(key: '/protected')->child(key: 'get')->value(key: 'security'),
        );
        $rawSchema = \file_get_contents($outputFile);
        self::assertIsString($rawSchema);
        self::assertStringContainsString('security: []', $rawSchema);
    }
    ...
    {
        return new FakeTranslator(locale: 'en', messages: ['gian_tiaga.spiral_openapi.successful_response' => 'Successful response.', 'gian_tiaga.spiral_openapi.api_error' => 'API error.', 'gian_tiaga.spiral_openapi.additional_response' => 'Additional response.']);
    }
    private function assertGeneratedSpec(OpenApiSpecNode $spec, string $successDescription, string $errorDescription): void
    {
        ...
        self::assertSame(['System'], $healthGet->value(key: 'tags'));
        // Тег остаётся объявлением операции: маршрут без атрибута не получает пустой ключ tags,
        // который OpenAPI трактовал бы как объявленную группу без имени.
        self::assertFalse($usersGet->has(key: 'tags'));
        ...
        // Отбор по списку значений называется со скобками: PHP собирает массив только у такого
        // имени, а имя без них он схлопнул бы до последнего названного значения.
        self::assertSame('limit', $usersParameters->child(key: 1)->value(key: 'name'));
        self::assertSame('status[]', $usersParameters->child(key: 2)->value(key: 'name'));
        ...
        self::assertSame(
            ...
            $schemas->child(key: 'HealthResourceDataResponse')->child(key: 'properties')->value(key: 'data'),
        );
        self::assertSame(
            ['type' => 'array', 'items' => ['$ref' => '#/components/schemas/UserResource']],
            $schemas->child(key: 'UserResourceCollectionResponse')->child(key: 'properties')->value(key: 'data'),
        );
        self::assertFalse($paths->has(key: '/internal-docs'));
        // Маршрут живости объявляет HEAD рядом с GET: отдельной операцией HEAD не описывается,
    ...
    /**
     * Тело успешного ответа описано схемой компонентов, а операция только ссылается на неё:
     * форма обёртки лежит в одном месте и получает имя, пригодное для клиента API.
     */
    private function assertSuccessResponseReferencesComponentSchema(OpenApiSpecNode $operation, string $schemaName, OpenApiSpecNode $schemas): void
    {
        $schema = $operation->child(key: 'responses')->child(key: 200)->child(key: 'content')->child(key: 'application/json')->child(key: 'schema');
        self::assertSame(\sprintf('#/components/schemas/%s', $schemaName), $schema->value(key: '$ref'));
        self::assertFalse($schema->has(key: 'properties'));
        self::assertSame('object', $schemas->child(key: $schemaName)->value(key: 'type'));
    }
